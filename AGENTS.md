...
- Verify rendering changes by RUNNING `zig-out\bin\zig-diablo.exe --gameshot` and
  looking at the 11 PNGs in `shots\` (rampart, family portrait, spawn grove, portal,
  stat sheet, skills ×2, menu, death, victory, editor). Don't claim a visual change
  works without a shot.
