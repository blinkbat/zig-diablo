...
// Controller rumble.
//
// raylib's GLFW desktop backend stubs out SetGamepadVibration, so on Windows we drive
// XInput directly — resolving XInputSetState at runtime from whichever xinput DLL is
// present (cached), sidestepping import-lib/ABI concerns and leaving build.zig untouched.
// Elsewhere we fall back to raylib's API (works under SDL, harmless no-op otherwise).
//
// Each rumble is an `Event`: peak per motor plus a duration. Motors fade linearly, and
// overlapping events blend "strongest wins" so a big hit takes over a lingering buzz
// without a weak tick cutting a strong one short.

// XInput has a low-frequency ("heavy") and high-frequency ("buzz") motor. Player actions
// lean on buzz; impacts suffered lean on heavy; death swells both.
...
const Motor = struct {
    ...
    // A new pulse takes over only if at least as strong, right now, as what's still
    // playing — big impact overrides a fading buzz; a small tick never truncates a
    // bigger event mid-fade.
...
pub const Rumble = struct {
    ...
    // Advance envelopes by dt and command the motors. `active` gates output: pass
    // false with no controller or when paused, so the grip is silent while envelopes
    // still decay in the background.
...
fn setMotors(low: f32, high: f32) void {
    ...
    if (builtin.os.tag == .windows) {
        ...
        // Best effort: works under raylib's SDL backend, no-op under GLFW.
